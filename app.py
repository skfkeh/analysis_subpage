...
df.drop(columns=['Dep_Time'],inplace=True)'''
    ...
    with tab_DT:
        ...
        data_path_dt = f"{os.path.dirname(os.path.abspath(__file__))}/data.csv"
        data = pd.read_csv(data_path_dt)
        df_dt = pd.DataFrame(data)
        df_dt.drop('Unnamed: 0',axis=1,inplace=True)
        # 데이터 전처리
        
        X_dt = df_dt.drop('Price', axis=1)
        y_dt = df_dt.Price
        ...
        train_pred_dt = model_dt.predict(X_train_dt) 
        test_pred_dt = model_dt.predict(X_test_dt)
        
        predict_button_dt = st.button('예측')
        
        if predict_button_dt:        
            st.write(f'Train-set : {model_dt.score(X_train_dt, y_train_dt)}')
            st.write(f'Test-set : {model_dt.score(X_test_dt, y_test_dt)}')
        ...
        train_relation_square_dt = model_dt.score(X_train_dt, y_train_dt)
        test_relation_square_dt = model_dt.score(X_test_dt, y_test_dt)
        ...
        if SearchBtn_dt:
            ...
            fig.add_trace(go.Scatter(x=y_train_dt,y=y_test_dt, mode='markers',name='Actual'))
            fig.add_trace(go.Scatter(x=y_test_dt,y=test_pred_dt,mode='markers',name='Predict')) # mode='lines+markers'
    ...
    with tab_RF:
        ...
        data_path_rf = f"{os.path.dirname(os.path.abspath(__file__))}/data.csv"
        data_rf = pd.read_csv(data_path_rf)
        df_rf = pd.DataFrame(data_rf
        df_rf.drop('Unnamed: 0',axis=1,inplace=True)
        # 데이터 전처리
        ...
        train_pred_rf = model.predict(X_train_rf) 
        test_pred_rf = model.predict(X_test_rf)

        predict_button_rf = st.button('예측')
        
        if predict_button_rf:
            st.write(f'Train-set : {model_rf.score(X_train_rf, y_train_rf)}')
            st.write(f'Test-set : {model_rf.score(X_test_rf, y_test_rf)}')
        ...
        st.subheader('RMSE 비교') 
        train_relation_square_rf = mean_squared_error(y_train_rf, train_pred_rf, squared=False)
        test_relation_square_rf = mean_squared_error(y_test_rf, test_pred_rf) ** 0.5
        ...
        if SearchBtn_rf:
            fig = make_subplots(rows=1, cols=1, shared_xaxes=True)
            fig.add_trace(go.Scatter(x=y_train_rf,y=y_test_rf, mode='markers',name='Actual'))
            fig.add_trace(go.Scatter(x=y_test_rf,y=test_pred_rf,mode='markers',name='Predict')) # mode='lines+markers'
            fig.update_layout(title='<b>actual과 predict 비교')
            st.plotly_chart(fig)
